...
public class BizFacade {
	...
	public void appInit() {
		...
		for (int i = 0; i < AppData.getAccountBooks().size(); i++) {
			reloadAccountBookInfo(AppData.getAccountBooks().get(i)
					.getAccountBookId());
		}
	...
	private void checkAndCreateDefaultInfo() {
		...
		for (int i = 0; i < AppData.getAccountBooks().size(); i++) {
			if (AppData.getAccountBooks().get(i).getAccountBookId() == AppData
		...
		if (!hasCurrentAccountBook) {
			// 没有当前账本，则设置第一个账本
			AppData.setCurrentAccountBookId(AppData.getAccountBooks().get(0)
					.getAccountBookId());
		}
	}
	...
	private void createDefaultCategory(int accountBookId) {
		...
		createCategory(accountBookId, "支出", "餐饮",
				ResourceMapper.icon_category_32);
		createCategory(accountBookId, "支出", "交通",
				ResourceMapper.icon_category_2);
		createCategory(accountBookId, "支出", "购物",
				ResourceMapper.icon_category_34);
		createCategory(accountBookId, "支出", "水果零食",
				ResourceMapper.icon_category_41);
		createCategory(accountBookId, "支出", "酒水饮料",
				ResourceMapper.icon_category_44);
		createCategory(accountBookId, "支出", "居家",
				ResourceMapper.icon_category_37);
		createCategory(accountBookId, "支出", "手机通讯",
				ResourceMapper.icon_category_40);
		createCategory(accountBookId, "支出", "报销账",
				ResourceMapper.icon_category_31);
		createCategory(accountBookId, "支出", "借出",
				ResourceMapper.icon_category_36);
		createCategory(accountBookId, "支出", "娱乐",
				ResourceMapper.icon_category_47);
		createCategory(accountBookId, "支出", "淘宝",
				ResourceMapper.icon_category_43);
		createCategory(accountBookId, "支出", "人情礼物",
				ResourceMapper.icon_category_39);
		createCategory(accountBookId, "支出", "医疗教育",
				ResourceMapper.icon_category_46);
		createCategory(accountBookId, "支出", "书籍",
				ResourceMapper.icon_category_42);
		createCategory(accountBookId, "支出", "美容运动",
				ResourceMapper.icon_category_38);
		createCategory(accountBookId, "支出", "宠物",
				ResourceMapper.icon_category_33);

		createCategory(accountBookId, "", "收入", ResourceMapper.icon_category_21);
		createCategory(accountBookId, "收入", "工资",
				ResourceMapper.icon_category_21);
		createCategory(accountBookId, "收入", "奖金",
				ResourceMapper.icon_category_23);
		createCategory(accountBookId, "收入", "外快兼职",
				ResourceMapper.icon_category_24);
		createCategory(accountBookId, "收入", "借入",
				ResourceMapper.icon_category_25);
		createCategory(accountBookId, "收入", "投资收入",
				ResourceMapper.icon_category_29);
		createCategory(accountBookId, "收入", "红包",
				ResourceMapper.icon_category_22);
		createCategory(accountBookId, "收入", "零花钱",
				ResourceMapper.icon_category_26);
		createCategory(accountBookId, "收入", "生活费",
				ResourceMapper.icon_category_28);
		createCategory(accountBookId, "收入", "其他",
				ResourceMapper.icon_category_27);
	...
	public AccountBook getCurrentAccountBook() {
		...
		for (int i = 0; i < accountBooks.size(); i++) {
			if (accountBooks.get(i).getAccountBookId() == AppData
					.getCurrentAccountBookId()) {
	...
	public AccountBook editAccountBooksDetail(int accountBookId, String name,
			String description) {
	...
	public void deleteAccountBook(int accountBookId) {
		...
		for (int i = 0; i < accountBooks.size(); i++) {
			...
			if (accountBookId == accountBooks.get(i).getAccountBookId()) {
				...
				if (AppData.getLastSyncDataTime() > accountBook.getCreateTime()
						.getTime()) {
	...
	public void reloadAccountBookInfo(int accountBookId) {
		...
		info.setExpendCategories(getSubCategories(accountBookId,
				Constant.CATEGORY_EXPEND));
		info.getExpendCategories().add(
				getCategory(accountBookId, Constant.CATEGORY_EXPEND));

		info.setIncomeCategories(getSubCategories(accountBookId,
				Constant.CATEGORY_INCOME));
		info.getIncomeCategories().add(
				getCategory(accountBookId, Constant.CATEGORY_INCOME));
		...
		statisticalInfo.setExpend(DBService.getInstance(App.getInstance())
				.sumWaterValueByIdAndCategories(accountBookId,
						info.getExpendCategories()));
		statisticalInfo.setIncome(DBService.getInstance(App.getInstance())
				.sumWaterValueByIdAndCategories(accountBookId,
						info.getIncomeCategories()));
		statisticalInfo.setBalance(statisticalInfo.getExpend()
				+ statisticalInfo.getIncome());

		info.setStatisticalInfo(statisticalInfo);

		// 成员用户信息
		List<UserInfo> memberUserInfos = new ArrayList<UserInfo>();
		List<Integer> memberUserIds = AppData.getBookMembersMap().get(
				accountBookId);
		for (int i = 0; null != memberUserIds && i < memberUserIds.size(); i++) {
			memberUserInfos.add(obtainUserInfo(null, memberUserIds.get(i),
					false));
		}
	...
	public AccountCategory createCategory(int accountBookId,
			String superCategory, String category, String icon) {
	...
	public AccountCategory getCategory(int accountBookId, String category) {
		for (int i = 0; i < AppData.getCategories().size(); i++) {
			...
			if (accountBookId == accountCategory.getAccountBookId()
					&& accountCategory.getCategory().equals(category)) {
	...
	public List<AccountCategory> getSubCategories(int accountBookId,
			String category) {
		List<AccountCategory> subAccountCategories = new ArrayList<AccountCategory>();
		for (int i = 0; i < AppData.getCategories().size(); i++) {
			AccountCategory accountCategory = AppData.getCategories().get(i);
			if (accountBookId == accountCategory.getAccountBookId()
					&& accountCategory.getSuperCategory().equals(category)) {
	...
	public AccountCategory getRootCategory(int accountBookId, String category) {
		...
		if (accountCategory == null
				|| accountCategory.getSuperCategory() == null
				|| accountCategory.getSuperCategory().isEmpty()) {
			return accountCategory;
		} else {
			return getRootCategory(accountBookId,
					accountCategory.getSuperCategory());
		}
	}

	public AccountCategory editCategory(int accountBookId, String category,
			String newCategory) {
		AccountCategory accountCategory = getCategory(accountBookId, category);
		if (null != accountCategory) {
			...
			// 修改子分类的父分类
			for (int i = 0; i < AppData.getCategories().size(); i++) {
				if (AppData.getCategories().get(i).getSuperCategory()
						.equals(category)) {
					AppData.getCategories().get(i)
							.setSuperCategory(newCategory);
				}
			}
	...
	// 账本条目相关接口
	public List<AccountRecord> getMoreAccountRecords(int accountBookId,
			long beginTime) {
		return DBService.getInstance(App.getInstance())
				.queryMoreAccountRecords(accountBookId, beginTime);
	}

	public void createRecord(AccountRecord record) {
	...
	public void editRecord(AccountRecord record) {
	...
	public void deleteRecord(AccountRecord record) {
		// 删除数据库中的数据
		DBService.getInstance(App.getInstance()).deleteAccountRecord(
				record.getAccountRecordId());

		// 添加进删除列表
		if (record.getCreateTime().getTime() < AppData.getLastSyncDataTime()) {
			// 是在同步之前创建的，则添加进删除缓存
			AppData.getDeleteRecords().add(record);
			AppData.setDeleteRecords(AppData.getDeleteRecords());
		}
	...
	private boolean isLocalInvalidBook(int accountBookId) {
		if (accountBookId == Constant.DEFAULT_ACCOUNT_BOOK_ID) {
			// 是默认账本
			if (DBService.getInstance(App.getInstance()).queryAccountCount(
					accountBookId) == 0) {
				// 无效的账本
				return true;
			}
	...
	private BoolResult<SyncDataRequestData> buildSyncDataRequestData(
			Context context) {
		...
		List<AccountBook> serverBooks = pullAccountBooksResponse.getData()
				.getBooks();
		...
		// 获得需要同步的账本记录
		List<AccountRecord> newRecords = dbService
				.queryNewAccountRecords(lastSyncTime); // 新建记录
		List<AccountRecord> updateRecords = dbService
				.queryUpdateAccountRecords(lastSyncTime); // 更新记录
	...
	public void syncDataFromServer(SyncDataResponseData responseData) {
		...
		if (responseData.isUpdateBooks()) {
			...
			// 如果当前账本是新账本，映射为新账本ID
			if (responseData.getNewBookIdMap().containsKey(
					AppData.getCurrentAccountBookId())) {
				AppData.setCurrentAccountBookId(responseData.getNewBookIdMap()
						.get(AppData.getCurrentAccountBookId()));
			}

			// 更新账本成员信息
			AppData.setBookMembersMap((HashMap<Integer, List<Integer>>) responseData
					.getBookMembersMap());
			...
			if (!isInvalidBook) {
				// 不是有效的，选择第一个
				AppData.setCurrentAccountBookId(AppData.getAccountBooks()
						.get(0).getAccountBookId());
			}
		}
		...
		// 删除本地的删除缓存
		AppData.setDeleteBooks(new ArrayList<AccountBook>());
		AppData.setDeleteCategories(new ArrayList<AccountCategory>());
		AppData.setDeleteRecords(new ArrayList<AccountRecord>());
	...
	public void syncDataToServer(Context context) {
		...
		ServerResponse<SyncDataResponseData> response = cmd.send(
				App.getInstance(), requestData, false);
					...
					MainThreadExecutor.run(new Runnable() {
						...
						public void run() {
							...
							String msg = String.format(
									"本地的账本\"%s\"的名称与服务器上的账本名称有冲突，换一个名字吧",
									localBook.getName());
							dialogBuilder
									.withButton2Drawable(
											R.drawable.selector_btn_inverse)
											...
											new View.OnClickListener() {
												...
												public void onClick(View v) {
													...
													transData
															.setType(Type.EDIT);
													transData
															.setAccountBook(localBook);

													Intent intent = new Intent(
															context,
															AccountBookEditActivity.class);
													context.startActivity(AccountBookEditActivity
															.makeTransDataIntent(
																	intent,
																	transData));
	...
	// 拉取账本信息
	public ServerResponse<PullAccountBooksResponseData> pullAccountBooks(
			Context context) {
		PullAccountBooksServerCmd cmd = new PullAccountBooksServerCmd();
		ServerResponse<PullAccountBooksResponseData> response = cmd.send(
				context, new PullAccountBooksRequestData(), false);
		return response;
	}
	 ...
	 * @param mail
	 */
	public ServerResponse<NullResponseData> verifyMail(Context context,
			String mail) {
	 ...
	 * @return
	 */
	public ServerResponse<NullResponseData> verifyVfCode(Context context,
			String username, int accountType, int vfCode) {
	 ...
	 * @return
	 */
	public ServerResponse<NullResponseData> register(Context context,
			String username, String password, int accountType, int vfCode) {
	 ...
	 * @return
	 */
	public ServerResponse<AccountLoginResponseData> accountLogin(
		...
		ServerResponse<AccountLoginResponseData> response = new AccountLoginServerCmd()
				.send(context, requestData, false);
		if (response.getRet() == ServerErrorCode.RET_SUCCESS) {
	...
	private void changeRecordsUserId(int oldUserId, int newUserId) {
		DBService.getInstance(App.getInstance()).updateAccountRecordsUserId(
				oldUserId, newUserId);
	}
	...
	public ServerResponse<NullResponseData> inviteMember(Context context,
			int accountBookId, String username) {
		InviteMemberRequestData requestData = new InviteMemberRequestData();
		requestData.setAccountBookId(accountBookId);
		requestData.setAccountBookName(getAccountBookById(accountBookId)
				.getName());
	...
	public ServerResponse<NullResponseData> agreeInviteMember(Context context,
			int accountBookId) {
	...
	public ServerResponse<PullUserInfosResponseData> pullUserInfos(
			Context context, List<Integer> userIds) {
	...
	public UserInfo obtainUserInfo(Context context, int userId,
			boolean pullWhenNoFound) {
		UserInfo info = AppData.getUserInfoCache().get(userId);
		if (info == null) {
			if (pullWhenNoFound) {
				List<Integer> userIds = new ArrayList<Integer>();
				userIds.add(userId);
				ServerResponse<PullUserInfosResponseData> response = pullUserInfos(
						context, userIds);
				if (response.getRet() == ServerErrorCode.RET_SUCCESS
						&& response.getData().getUserInfos().size() > 0) {
	 ...
	 * @return
	 */
	public long obtainNowUpdateTime() {
		long now = System.currentTimeMillis();
		if (now < AppData.getLastSyncDataTime()) {
			// 总不能比更新时间还 小吧，是这种情况的话，将会被漏过无法更新
			now = AppData.getLastSyncDataTime() + 1;
		}
	...
	public boolean pullAndHandleMsgs(Context context) {
		...
		ServerResponse<PullMsgsResponseData> response = cmd.send(context,
				new PullMsgsRequestData(), false);
		if (response.getRet() == ServerErrorCode.RET_SUCCESS) {
	...
	public void doInviteMembers(final Context context,
			final List<InviteMemberInfo> infos) {
		if (infos.size() == 0) {
			return;
		}
		...
		for (int i = 0; i < infos.size(); i++) {
			...
			final UserInfo userInfo = obtainUserInfo(context,
					info.getInviterUserId(), true);
			MainThreadExecutor.run(new Runnable() {
				...
				public void run() {
					final CustomDialogBuilder dialogBuilder = CustomDialogBuilder
							.getInstance(context);
					String msg = String.format("%s 邀请您一起记录账本\"%s\"，要加入吗？",
							userInfo.getNickname(), info.getAccountBookName());
					dialogBuilder.withMessage(msg).withButton1Text("爽快加入")
							...
							.setButton1Click(new View.OnClickListener() {
								...
								public void onClick(View v) {
									new Thread(new Runnable() {
										...
										@Override
										public void run() {
											agreeInviteMember(context,
													info.getAccountBookId());
