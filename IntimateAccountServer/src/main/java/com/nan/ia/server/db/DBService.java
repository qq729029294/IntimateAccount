...
public class DBService {
	...
	public boolean insertCategories(List<AccountCategory> categories) {
		// 新账本
		List<AccountCategoryTbl> newCategoryTbls = EntitySwitcher.fromItemsForDB(categories.toArray(a));
		...
		Session session = HibernateUtil.getSession();
		Transaction trans = session.beginTransaction();
		for (int i = 0; i < newCategoryTbls.size(); i++) {
			session.save(newCategoryTbls.get(i));
		}
	...
	public boolean deleteCategories(List<AccountCategoryDelete> deletes) {
		// 新账本
		List<AccountCategoryDeleteTbl> deleteTbls = EntitySwitcher.toCategoryDeleteTblsForDB(deletes);
		if (deleteTbls.size() == 0) {
			return true;
		}

		Session session = HibernateUtil.getSession();
		Transaction trans = session.beginTransaction();
		for (int i = 0; i < deleteTbls.size(); i++) {
			session.save(deleteTbls.get(i));
		}
		...
		try {
			...
			if (null != requestData.getBookDeletes()) {
				for (int i = 0; i < requestData.getBookDeletes().size(); i++) {
					AccountBookDelete item = requestData.getBookDeletes()
							.get(i);
			...
			for (int i = 0; i < accountBookDeleteTbls.size(); i++) {
				...
				// 同时删除账本下的类别
				deleteCategoryByAccountBookId(accountBookDeleteTbls.get(i)
						.getAccountBookId());
				// 同时删除账本下的账目
				deleteAccountItemByAccountBookId(accountBookDeleteTbls.get(i)
						.getAccountBookId());
			}
			...
			// 添加书的创建者为书的成员
			for (int i = 0; i < newAccountBookTbls.size(); i++) {
				insertAccountBookMember(newAccountBookTbls.get(i)
						.getCreateUserId(), newAccountBookTbls.get(i)
						.getAccountBookId());
			}
	...
	public BoolResult<UserTbl> getUser(int userId) {
		try {
			...
			// 检查是否已经存在用户名
			Query query = session
					.createQuery("FROM UserTbl r WHERE r.user_id = ?");
			query.setParameter(0, userId);
